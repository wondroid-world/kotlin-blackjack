...
class DealerTest {
    ...
    fun `딜러는 이름과 카드들을 가진다`() {
        val handCards =
            HandCards.from(Card(CardShape.HEART, Denomination.FIVE), Card(CardShape.CLOVER, Denomination.TWO))
        val dealer = Dealer(handCards = handCards)

        assertAll({ assertThat(dealer.name).isEqualTo("딜러") }, { assertThat(dealer.handCards).isEqualTo(handCards) })
    }
    ...
    fun `딜러의 카드 리스트의 초기값은 비어있다`() {
        ...
        assertThat(dealer.handCards.getHandCardsInfo().size).isEqualTo(expected)
    }
